...
// MARK: - SDKFido2UserInterfaceTests

/// Tests for `SDKFido2UserInterface`.
///
class SDKFido2UserInterfaceTests: BitwardenTestCase {
    // MARK: Properties

    var subject: SDKFido2UserInterface!

    // MARK: Setup & Teardown

    override func setUp() {
        super.setUp()
        subject = SDKFido2UserInterface()
    }
    ...
    func test_pickCredentialForAuthentication_empty_throwsNoMatchingCredential() async {
        await assertAsyncThrows(error: SDKPasskeyError.noMatchingCredential) {
    ...
    func test_pickCredentialForAuthentication_multiple_throwsAmbiguousCredential() async {
        await assertAsyncThrows(error: SDKPasskeyError.ambiguousCredential) {
