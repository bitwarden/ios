...
- **Snapshot tests — pick the approach with the user** — there are two valid patterns, and which fits is a human-discretion call. (1) Instantiate the view directly: build it with a `MockProcessor`/`Store` in `setUp`, then `assertSnapshot(of: subject.navStackWrapped, as:)` per state (all prefixed `disabletest_`); see `AddEditSendItemView+SnapshotTests.swift`. This is required for new views, which use `#Preview` macros that the snapshot harness cannot enumerate. (2) Iterate `PreviewProvider._allPreviews`, which is sometimes the better path for views that still expose a `PreviewProvider`. When a view could go either way, ask the user which approach to use rather than deciding unilaterally.
