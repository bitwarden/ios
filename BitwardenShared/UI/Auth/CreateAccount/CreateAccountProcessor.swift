...
class CreateAccountProcessor: StateProcessor<CreateAccountState, CreateAccountAction, CreateAccountEffect> {
    ...
    private func createAccount(captchaToken: String? = nil) async {
        ...
        do {
            ...
            let keys = try await services.clientService.clientAuth(for: userId).makeRegisterKeys(
                email: email,
                password: state.passwordText,
                kdf: kdf
            )

            let hashedPassword = try await services.clientService.clientAuth(for: userId).hashPassword(
                email: email,
                password: state.passwordText,
                kdfParams: kdf,
