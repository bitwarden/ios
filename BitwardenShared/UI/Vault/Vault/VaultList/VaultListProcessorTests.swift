...
class VaultListProcessorTests: BitwardenTestCase { // swiftlint:disable:this type_body_length
    ...
    /// the user's subscription is active (not past due).
    @MainActor
    func test_perform_streamVaultList_subscriptionNeedsAttentionCard_active_hidden() async throws {
        stateService.doesActiveAccountHavePremiumPersonallyResult = true
        billingService.getSubscriptionReturnValue = .fixture(status: .active)
        let section = VaultListSection(id: "1", items: [.fixture()], name: "Section")
    ...
    /// `perform(_:)` with `.streamVaultList` hides the subscription needs attention card and
    /// skips the network call when the user does not have premium personally.
    @MainActor
    func test_perform_streamVaultList_subscriptionNeedsAttentionCard_noPremiumPersonally_hidden() async throws {
        stateService.doesActiveAccountHavePremiumPersonallyResult = false
        let section = VaultListSection(id: "1", items: [.fixture()], name: "Section")
    ...
    /// `perform(_:)` with `.streamVaultList` hides the subscription needs attention card when
    /// `getSubscription()` throws.
    @MainActor
    func test_perform_streamVaultList_subscriptionNeedsAttentionCard_fetchError_hidden() async throws {
        stateService.doesActiveAccountHavePremiumPersonallyResult = true
        billingService.getSubscriptionThrowableError = BitwardenTestError.example
        let section = VaultListSection(id: "1", items: [.fixture()], name: "Section")
        ...
        vaultRepository.vaultListSubject.send(VaultListData(sections: [section]))
        try await waitForAsync { self.billingService.getSubscriptionCalled }
    ...
    /// `perform(.profileSwitcher(.requestedProfileSwitcher))`
    /// for iOS 26 navigates to present the profile switcher sheet.
    @MainActor
    func test_perform_requestedProfileSwitcher_iOS26() async throws {
        ...
        // Ensure the coordinator navigates to present the profile switcher
    ...
    /// lock the active account, dismissing the profile switcher after confirmation.
    @MainActor
    func test_receive_accountLongPressed_lock_activeAccount_iOS26() async throws {
        guard #available(iOS 26, *) else {
            throw XCTSkip("This test requires iOS 26 or later")
        }
    ...
    /// `receive(_:)` with `.profileSwitcher(.accountLongPressed)` for iOS 26 shows the alert and allows the user to
    /// lock the other account, which displays a toast and dismisses the profile switcher.
    @MainActor
    func test_receive_accountLongPressed_lock_otherAccount_iOS26() async throws {
        guard #available(iOS 26, *) else {
            throw XCTSkip("This test requires iOS 26 or later")
        }
    ...
    /// `receive(_:)` with `.profileSwitcher(.accountLongPressed)` for iOS 26 shows the alert and allows the user to
    /// log out of the active account, dismissing the profile switcher after confirmation.
    @MainActor
    ...
    /// `receive(_:)` with `.profileSwitcher(.accountLongPressed)` for iOS 26 shows the alert and allows the user to
    /// log out of the other account, which displays a toast and dismisses the profile switcher.
    @MainActor
    ...
    /// `receive(_:)` with `.addAccountPressed` for iOS 26 dismisses the profile switcher.
    @MainActor
    func test_receive_accountPressed_iOS26() async throws {
        guard #available(iOS 26, *) else {
            throw XCTSkip("This test requires iOS 26 or later")
        }
    ...
    @MainActor
    func test_receive_profileSwitcherBackgroundPressed_iOS26() throws {
        guard #available(iOS 26, *) else {
            throw XCTSkip("This test requires iOS 26 or later")
        }
