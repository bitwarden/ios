...
class ExportVaultProcessorTests: BitwardenTestCase { // swiftlint:disable:this type_body_length
    ...
    /// `.receive()` with `.exportVaultTapped` calls the export service with empty restricted types when no policies apply.
    @MainActor
    func test_receive_exportVaultTapped_noRestrictedTypes() async throws {
        let testURL = URL(string: "www.bitwarden.com")!
        exportService.exportVaultContentResult = .success("")
        exportService.writeToFileResult = .success(testURL)
        ...
        XCTAssertEqual(exportService.exportVaultContentsRestrictedTypes, [])
        XCTAssertEqual(coordinator.routes.last, .shareURL(testURL))
    }

    /// `.receive()` with `.exportVaultTapped` calls the export service with card type restricted when policy applies.
    @MainActor
    func test_receive_exportVaultTapped_withRestrictedTypes() async throws {
        let testURL = URL(string: "www.bitwarden.com")!
        exportService.exportVaultContentResult = .success("")
        exportService.writeToFileResult = .success(testURL)
        policyService.policyAppliesToUserPolicies = [
        ...
        XCTAssertEqual(exportService.exportVaultContentsRestrictedTypes, [.card])
        XCTAssertEqual(coordinator.routes.last, .shareURL(testURL))
    }

    /// `.receive()` with `.exportVaultTapped` applies restricted types for encrypted JSON format.
    @MainActor
    func test_receive_exportVaultTapped_encryptedJsonWithRestrictedTypes() async throws {
        let testURL = URL(string: "www.bitwarden.com")!
        exportService.exportVaultContentResult = .success("")
        exportService.writeToFileResult = .success(testURL)
        policyService.policyAppliesToUserPolicies = [
    ...
    /// `.receive()` with `.exportVaultTapped` applies restricted types for CSV format.
    @MainActor
    func test_receive_exportVaultTapped_csvWithRestrictedTypes() async throws {
        let testURL = URL(string: "www.bitwarden.com")!
        exportService.exportVaultContentResult = .success("")
        exportService.writeToFileResult = .success(testURL)
        policyService.policyAppliesToUserPolicies = [
        ...
        XCTAssertEqual(exportService.exportVaultContentsFormat, .csv)
        XCTAssertEqual(exportService.exportVaultContentsRestrictedTypes, [.card])
        XCTAssertEqual(coordinator.routes.last, .shareURL(testURL))
    }
