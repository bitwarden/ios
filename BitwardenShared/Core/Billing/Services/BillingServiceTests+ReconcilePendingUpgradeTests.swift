...
// MARK: - BillingServiceTests+ReconcilePendingUpgradeTests
...
extension BillingServiceTests {
    ...
    func start_abandonsStaleReconcileOnAccountSwitch() async throws {
        ...
        // `reconcilePendingUpgradeIfNeeded(userId:)` re-checks the active account twice: once
        // before the flag-clearing write below, then again after `doesActiveAccountHavePremium()`
        // where this test's switch actually lands. Waiting for the second call confirms both
        // checks — and the bail this test is asserting on — have run.
        try await waitForAsync { stateService.getActiveAccountIdCallCount == 2 }
        #expect(stateService.setUpgradedToPremiumActionCardVisibleCallCount == 0)
        #expect(stateService.premiumUpgradePendingResult == true)
    }

    /// `start()` also abandons an in-flight reconcile without writing anything if the active
    /// account changes while it's suspended reading the pending/failed flags themselves, before
    /// `doesActiveAccountHavePremium()` is ever reached — the earlier of the two account-identity
    /// re-checks in `reconcilePendingUpgradeIfNeeded(userId:)` catches this, rather than letting
    /// `setPremiumUpgradeLastSyncAttemptFailed(false)` clear the newly-active account's own flag.
    @Test
    func start_abandonsStaleReconcileOnAccountSwitchDuringPendingFlagRead() async throws {
        stateService.activeAccount = .fixture(profile: .fixture(userId: "1"))
        stateService.premiumUpgradeLastSyncAttemptFailedResult = true
        stateService.getPremiumUpgradePendingHandler = {
            // Simulates the account switch landing exactly inside this suspension window.
            stateService.activeAccount = .fixture(profile: .fixture(userId: "2"))
        }
