...
class KdfConfigProtocolTests: BitwardenTestCase {
    ...
    func test_pbkdf2sha256() {
        let subject = KdfConfig(kdfType: .pbkdf2sha256, kdfIterations: 600_000)
    ...
    func test_argon2id_allValues() {
        let subject = KdfConfig(kdfType: .argon2id, kdfIterations: 600_000, kdfMemory: 32, kdfParallelism: 3)
        let sdkKdf = subject.sdkKdf
    ...
    func test_argon2id_nilMemory() {
        let subject = KdfConfig(kdfType: .argon2id, kdfIterations: 600_000, kdfMemory: nil, kdfParallelism: 3)
        let sdkKdf = subject.sdkKdf
    ...
    func test_argon2id_nilParallelism() {
        let subject = KdfConfig(kdfType: .argon2id, kdfIterations: 600_000, kdfMemory: 32, kdfParallelism: nil)
        let sdkKdf = subject.sdkKdf
