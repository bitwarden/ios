...
class CipherOwnerTests: XCTestCase {
    ...
    func test_isPersonal() {
        XCTAssertFalse(CipherOwner.organization(id: "1", name: "Organization").ownerType.isPersonal)
        XCTAssertTrue(CipherOwner.personal(email: "user@bitwarden.com").ownerType.isPersonal)
    }

    /// `OwnerType.organizationId` returns the organization ID of an organization owner.
    func test_organizationId() {
        XCTAssertEqual(CipherOwner.organization(id: "1", name: "Organization").ownerType.organizationId, "1")
        XCTAssertEqual(CipherOwner.organization(id: "2", name: "Organization").ownerType.organizationId, "2")
        XCTAssertNil(CipherOwner.personal(email: "user@bitwarden.com").ownerType.organizationId)
    }
}
