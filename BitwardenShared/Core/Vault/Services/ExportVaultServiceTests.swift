...
final class ExportVaultServiceTests: BitwardenTestCase { // swiftlint:disable:this type_body_length
    ...
    override func setUp() {
        ...
        clientService.mockExporters.exportVaultReturnValue = "success"
    ...
    func test_exportVaultFileContents_csv() async throws {
        ...
        XCTAssertEqual(clientService.mockExporters.exportVaultReceivedArguments?.folders, [folder])
        XCTAssertEqual(
            clientService.mockExporters.exportVaultReceivedArguments?.ciphers,
            [
                archivedCipher,
                loginCipher,
                secureNoteCipher,
    ...
    func test_exportVaultFileContents_encryptedJSON() async throws {
        ...
        XCTAssertEqual(clientService.mockExporters.exportVaultReceivedArguments?.folders, [folder])
        XCTAssertEqual(
            Set(clientService.mockExporters.exportVaultReceivedArguments?.ciphers ?? []),
            Set([
                archivedCipher,
                cardCipher,
                loginCipher,
    ...
    func test_exportVaultFileContents_error_export() async throws {
        clientService.mockExporters.exportVaultThrowableError = BitwardenTestError.example
        await assertAsyncThrows(error: BitwardenTestError.example) {
            _ = try await subject.exportVaultFileContents(format: .csv, includeArchivedItems: true)
        }
    }
