...
class StateService {
    static let shared: StateService = .init()
...
//    let TIMEOUT_ACTION_KEY = "timeout_action_key"
    ...
    /// Checks the state integrity of the app. If the app has been reinstalled then there are some keychain things that
    /// need to be reset.
    func checkIntegrity() {
        if hasRunBefore {
            return
    ...
    var lackedDeviceOwnerAuthLastTime: Bool {
        get {
            UserDefaults.standard.bool(forKey: LACKED_DEVICE_OWNER_AUTH_LAST_TIME_KEY)
        }
        set(newValue) {
